...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    code: ({ children }) => (
      <code className="bg-foreground/5 rounded px-1.5 py-0.5 text-sm font-mono">{children}</code>
    ),
    pre: ({ children }) => (
      <pre className="bg-foreground/5 rounded-lg p-4 overflow-x-auto my-4 text-sm">{children}</pre>
    ),
    a: ({ href, children }) => (
      <a
        href={href}
