...
export function getAllWeeks(): WeekPost[] {
  ...
  const processMdxFile = (filePath: string, displayName: string) => {
    ...
    if (process.env.NODE_ENV === "development") {
      const validation = validateWeekDates(displayName, frontmatter.startDate, frontmatter.endDate);
  ...
  const processTxtFile = (filePath: string, displayName: string) => {
    ...
    // Validate dates in development
    if (process.env.NODE_ENV === "development") {
      const validation = validateWeekDates(displayName, startDate, endDate);
      if (!validation.isValid) {
        console.warn(`\n⚠️  Date validation failed for ${displayName}:`);
        validation.errors.forEach((err) => console.warn(`   - ${err}`));
        console.warn("");
